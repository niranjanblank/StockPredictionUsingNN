...
def value_table():
   ...
   for i in range(10):
      ...
      l1 = Label(value_window, text=data.iloc[i][0], relief="ridge", width=20, pady=5)
      l1.grid(row=i + 4, column=2)
      l1 = Label(value_window, text=data.iloc[i][1], relief="ridge", width=20, pady=5)
      l1.grid(row=i + 4, column=3)
      l1 = Label(value_window, text=data.iloc[i][2], relief="ridge", width=20, pady=5)
      l1.grid(row=i + 4, column=4)
      l1 = Label(value_window, text=data.iloc[i][3], relief="ridge", width=20, pady=5)
      l1.grid(row=i + 4, column=5)
      l1 = Label(value_window, text=data.iloc[i][4], relief="ridge", width=20, pady=5)
      l1.grid(row=i + 4, column=6)
      l1 = Label(value_window, text=stock_one[i], pady=5, relief="ridge", width=25)
      l1.grid(row=i + 4, column=7)
      l1 = Label(value_window, text=stock_three[i], pady=5, relief="ridge", width=25)
...
def check():
   ...
   predview = Label(screen, text="Predicted Value", fg="green", font=("Calibri", 15)).pack()
   predview2 = Label(screen, text=predicted).pack()



  # Label(text=y,fg="green",font = ("Calibri",15)).pack()
def predict_screen():
   global screen
 ...
 #  Label( text="Company ID").pack()
  ...
  # id_entry.pack()
   ...
   volume_entry.pack()
   Label(screen,text="").pack()
   Button(screen,text="Predict",bg="grey",fg="black", width=10, height=1, command=check).pack()
   Label(screen,text="").pack()
   Label(screen,text="").pack()
...
def helloCallBack():
   ...
   L1 = Label(login, text="user name")
   L1.grid(row=1, column=0)
   L2 = Label(login, text="password")
   L2.grid(row=2, column=0)
   E1 = Entry(login, bd=5)
   E1.grid(row=1, column=1)
   E2 = Entry(login, bd=5, show="*")
   E2.grid(row=2, column=1)
   Blogin = Button(login, text="login", activeforeground="red")
   Blogin.grid(columnspan=2, sticky=S)
   login.mainloop()
...
def data_frame():
   ...
   win.title("data")
   l1 = Label(win, text=" S.N.  ")
   l1.grid(row=1, column=0)
   l1 = Label(win, text=" Traded Companies ")
   l1.grid(row=1, column=1)
   l1 = Label(win, text=" Open price ")
   l1.grid(row=1, column=2)
   l1 = Label(win, text=" Close Price ")
   l1.grid(row=1, column=3)
   l1 = Label(win, text=" Highest price ")
   l1.grid(row=1, column=4)
   l1 = Label(win, text=" Lowest Price ")
   l1.grid(row=1, column=5)
   l1 = Label(win, text="  Volume  ")
   l1.grid(row=1, column=6)
   win.mainloop()


photo = ImageTk.PhotoImage(Image.open('Photos/test.png'))
...
B = Button(frame, text ="Admin Login", command=helloCallBack, height=1, activeforeground="red")
B.pack(side=LEFT)

B1 = Button (bottomframe, text="Predict Using Your Values", command = predict_screen )
B1.pack(side=LEFT)

B2 = Button (bottomframe, text="Predicted Values",command = value_table)
B2.pack(side=LEFT)

B3 = Button (bottomframe, text="Stock Market Condition", command=main_screen)
B3.pack(side=LEFT)
